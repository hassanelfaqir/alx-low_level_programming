...
void print_entry(unsigned long int e_er, unsigned char *e_id);
 ...
 * @e_er: The address of the ELF entry point.
 * @e_id: A pointer to an array containing the ELF class.
 */
void print_entry(unsigned long int e_er, unsigned char *e_id)
{
	...
	if (e_id[EI_DATA] == ELFDATA2MSB)
	{
		e_er = ((e_er << 8) & 0xFF00FF00) |
			  ((e_er >> 8) & 0xFF00FF);
		e_er = (e_er << 16) | (e_er >> 16);
	}
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	print_type(header->e_typ, header->e_id);
	print_entry(header->e_er, header->e_id);
